...
const STUDENTS_PER_PAGE_EARLY = 20;
...
export default function PrimaryLogExporter({ classes, settings }: PrimaryLogExporterProps) {
    ...
    const handleExportEarlyGradesPdf = async (examType: 'midYear' | 'finalYear') => {
        ...
        const allPagesData: { classData: ClassData, students: Student[], startingIndex: number, isLastPageOfClass: boolean }[] = [];

        for (const classData of classesToExport) {
            const students = [...(classData.students || [])].sort((a,b) => a.name.localeCompare(b.name, 'ar'));
            if (students.length === 0) {
                allPagesData.push({ classData, students: [], startingIndex: 0, isLastPageOfClass: true });
            } else {
                for (let i = 0; i < students.length; i += STUDENTS_PER_PAGE_EARLY) {
                    const isLast = (i + STUDENTS_PER_PAGE_EARLY) >= students.length;
                    allPagesData.push({
                        classData,
                        students: students.slice(i, i + STUDENTS_PER_PAGE_EARLY),
                        startingIndex: i,
                        isLastPageOfClass: isLast
                    });
                }
            }
        }
        ...
        try {
            ...
            for (let i = 0; i < totalPages; i++) {
                const { classData, students, startingIndex, isLastPageOfClass } = allPagesData[i];
                ...
                await renderComponent(
                    <PrimaryLogPDFPage
                        ...
                        isLastPageOfClass={isLastPageOfClass}
                    />
                );
