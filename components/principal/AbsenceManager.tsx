...
const STUDENTS_PER_PAGE_REPORT = 40;
    ...
    const handleExport = async (type: 'report' | 'letter', student?: Student) => {
        ...
        try {
            ...
            if (type === 'report' && selectedClass) {
                const studentChunks = [];
                for (let i = 0; i < sortedStudents.length; i += STUDENTS_PER_PAGE_REPORT) {
                    studentChunks.push(sortedStudents.slice(i, i + STUDENTS_PER_PAGE_REPORT));
                }
                
                // If no students, create one empty chunk to render at least one page
                ...
                for (let i = 0; i < studentChunks.length; i++) {
                    await renderComponent(
                        <MonthlyAbsenceReportPDF
                            ...
                            month={currentMonth}
                            startIndex={i * STUDENTS_PER_PAGE_REPORT}
                        />
                    );
