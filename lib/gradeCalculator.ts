...
export function calculateStudentResult(student: Student, subjects: Subject[], settings: SchoolSettings, classData: ClassData): { finalCalculatedGrades: Record<string, CalculatedGrade>, result: StudentResult } {
    ...
    // Pass 1: Calculate all annualPursuit values to check for General Exemption
    const allPursuits: Record<string, number | null> = {};
    subjects.forEach(subject => {
        const grade: SubjectGrade = { ...student.grades?.[subject.name] };
        ...
        if (isValidGrade(firstTerm) && isValidGrade(grade.midYear) && isValidGrade(secondTerm)) {
            allPursuits[subject.name] = Math.round((firstTerm! + grade.midYear! + secondTerm!) / 3);
        }
    });

    // Check for General Exemption (الاعفاء العام)
    const pursuitValues = Object.values(allPursuits).filter(isValidGrade);
    let isGeneralExempt = false;
    if (pursuitValues.length === subjects.length && subjects.length > 0) {
        const sum = pursuitValues.reduce((a, b) => a + b, 0);
        const average = Math.round(sum / subjects.length);
        const minGrade = Math.min(...pursuitValues);
        if (average >= 85 && minGrade >= 75) {
            isGeneralExempt = true;
        }
    }

    // Pass 2: Calculate final grades and apply exemptions
    subjects.forEach(subject => {
        ...
        const annualPursuit = allPursuits[subject.name] ?? null;

        // Individual Exemption (الاعفاء الفردي) criteria: Pursuit >= 90
        const isIndividualExempt = isValidGrade(annualPursuit) && annualPursuit >= 90;
        const isExempt = isGeneralExempt || isIndividualExempt;
    ...
    if (anySecondAttemptGradeEntered) {
        ...
        if (isGeneralExempt) {
            status = 'ناجح';
            message = 'معفو إعفاء عام';
        } else if (isMinisterial) {
